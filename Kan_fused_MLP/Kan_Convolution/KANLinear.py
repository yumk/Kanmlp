...
class KANLinear(torch.nn.Module):
    def __init__(
        ...
        self.spline_weight = torch.nn.Parameter(
            torch.Tensor(out_features, hidden_features, grid_size + spline_order)
        )
        if enable_standalone_scale_spline:  # 如果启用独立的分段多项式缩放，则初始化分段多项式缩放参数
            self.spline_scaler = torch.nn.Parameter(
                torch.Tensor(out_features, hidden_features)
            )
    ...
    def reset_parameters(self):
        ...
        with torch.no_grad():
            noise = (# 生成缩放噪声
                (
                    torch.rand(self.grid_size + 1, self.hidden_features, self.out_features)
                    - 1 / 2
                )
                * self.scale_noise
                / self.grid_size
    ...
    def curve2coeff(self, x: torch.Tensor, y: torch.Tensor):
        ...
        """
        # assert x.dim() == 2 and x.size(1) == self.in_features
        assert y.size() == (x.size(0), self.hidden_features, self.out_features)
        # 计算 B-样条基函数
